...
int copy_out(char *fname) {
  ...
  if (shellDiskFileSize > 0 && contentBuffer[shellDiskFileSize - 1] != '\n') {
    contentBuffer[shellDiskFileSize] = '\n';
    shellDiskFileSize++; // Increase file size to account for the added newline
  }
  fwrite(contentBuffer, sizeof(char), shellDiskFileSize, realDiskFile);
